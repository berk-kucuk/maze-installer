#!/usr/bin/env bash
#
# build.sh — build (and optionally install / publish) this Maze package in one shot.
#
#   ./build.sh             build  ->  ./<pkg>-<ver>.pkg.tar.zst
...
#                          (updates DIR/maze-aur.db.tar.zst — the ISO's localrepo)
#
...
PKGFILE=$(ls -t ./*.pkg.tar.* 2>/dev/null | head -1 || true)
...
if [ -n "$REPO" ]; then
  ...
  repo-add "$REPO/maze-aur.db.tar.zst" "$REPO/$(basename "$PKGFILE")"
fi
